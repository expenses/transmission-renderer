...
fn main() -> anyhow::Result<()> {
    ...
    let model_buffers = std::sync::Arc::new(model_staging_buffers.upload(&mut init_resources)?);

    // todo: reduce this it model.num_opaque + model2.num_opaque

    let draw_buffers = std::sync::Arc::new(DrawBuffers::new(max_draw_counts, &mut init_resources)?);
    ...
    let light_buffers = std::sync::Arc::new(LightBuffers {
        ...
        let loop_closure = || -> anyhow::Result<()> {
            match event {
                ...
                Event::RedrawRequested(_) => unsafe {
                    ...
                    let zero_frustum_culling_buffers = command_graph.register_commands("zero_frustum_culling_buffers", &[(reset_query_pool, None)], &device, move |command_buffer| {
                    ...
                    let light_assignment = command_graph.register_commands("light_assignment", &[(reset_query_pool, None)], &device, move |command_buffer| {
                    ...
                    command_graph.register_commands("render", &[(frustum_culling_demultiplex_draws, None), (light_assignment, None)], &device, move |command_buffer| {
                        record(RecordParams {
                            device,
                            command_buffer,
                            pipelines,
                            ...
                            descriptor_sets,
                            opaque_sampled_hdr_framebuffer_image,
...
struct RecordParams<'a> {
    ...
    opaque_sampled_hdr_framebuffer_image: vk::Image,
    descriptor_sets: DescriptorSets,
    dynamic: DynamicRecordParams,
}
...

    {
        ...
        vk_sync::cmd::pipeline_barrier(
            ...
            &[vk_sync::ImageBarrier {
                ...
                next_accesses: &[vk_sync::AccessType::TransferWrite],
                next_layout: vk_sync::ImageLayout::Optimal,
                image: opaque_sampled_hdr_framebuffer_image,
                range: subresource_range,
                discard_contents: true,
                ..Default::default()
            }],
